...
import { CheckCircle2, Download, FileSpreadsheet, FileText, Printer, Users } from 'lucide-svelte'
...
$effect(() => {
	...
	;(async () => {
		...
		const attempts = await convexQuery(api.tests.listAttemptsByTest, { testId }, [] as any[])
	...
	if (isConvexConfigured()) {
		try {
			if (selectedFormat === 'csv') {
				...
				const studentsCSV = await convexQuery(api.csv.exportStudents, { schoolId }, null as any)
