...
export default function StudentsScreen() {
	...
	const avgAttendance =
		...
		(filtered.length > 0
			? Math.round(filtered.reduce((acc: number, s: any) => acc + (s.attendance ?? 0), 0) / filtered.length)
			: 0)
			...
			>
				{students === undefined ? (
					...
					filtered.map((student: any) => {
						...
						const parentName: string = student.parentName ?? student.parent ?? student.user?.parentName ?? "—"
