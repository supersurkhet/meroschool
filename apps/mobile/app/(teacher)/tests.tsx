...
export default function TeacherTestsScreen() {
	...
	const avgScore =
		attemptList.length > 0
			? Math.round(
					attemptList.reduce((acc: number, r: any) => acc + (r.percentage ?? 0), 0) /
						attemptList.length,
				)
			: null
						...
						>
							<Text
								style={{
									fontSize: 13,
									fontWeight: '600',
									color: selectedSubjectId === subj._id ? '#FFF' : colors.textSecondary,
								}}
							>
			...
			>
				...
				{tab === 'tests' && !showCreate && !selectedSubjectId && (
					<EmptyState
						...
						subtitle="Pick a subject above to view its tests"
					/>
				)}
				...
				{tab === 'tests' &&
					...
					testList.length === 0 && (
						<EmptyState
							...
							subtitle="Create your first test using the + button above"
						/>
					)}
							...
							>
								<View style={{ gap: 6 }}>
									...
									<Text style={{ fontSize: 13, color: colors.textSecondary }}>
										{test.sectionName ?? test.class ?? '—'} ·{' '}
										{test.totalQuestions ?? test.questions ?? 0}Q ·{' '}
										{test.durationMinutes ? `${test.durationMinutes} min` : (test.duration ?? '—')}{' '}
										· {test.dueDate ?? test.date ?? ''}
									</Text>
								</View>
							</Card>
						</Pressable>
					...
					<>
						...
						{!selectedTestId && (
							<EmptyState
								icon="book-outline"
								title="Select a Test"
								subtitle="Pick a test in the Tests tab to view its question bank"
							/>
						)}
						...
						{selectedTestId && questions !== undefined && questionList.length === 0 && (
							<EmptyState
								...
								subtitle="Add questions to build your MCQ library"
							/>
						)}
									...
									<View style={{ flexDirection: 'row', gap: 8 }}>
										...
										{q.difficulty && (
											<View
												style={{
													backgroundColor:
														(difficultyColors[q.difficulty] ?? colors.textSecondary) + '20',
													paddingHorizontal: 8,
													paddingVertical: 2,
													borderRadius: 6,
												}}
											>
												<Text
													style={{
														fontSize: 11,
														fontWeight: '600',
														color: difficultyColors[q.difficulty] ?? colors.textSecondary,
													}}
												>
													{q.difficulty}
												</Text>
					...
					<>
						{!selectedTestId && (
							<EmptyState
								...
								title="Select a Test"
								subtitle="Pick a test in the Tests tab to view results"
							/>
						)}
						...
						{selectedTestId && attempts !== undefined && attemptList.length === 0 && (
							<EmptyState
								...
								subtitle="Results will appear after students take tests"
							/>
						)}
								...
								<Card>
									<View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
										<Text style={{ fontSize: 14, color: colors.textSecondary }}>
											{testList.find((t: any) => t._id === selectedTestId)?.title ?? 'Test'} ·{' '}
											{attemptList.length} students
										</Text>
										{avgScore !== null && (
