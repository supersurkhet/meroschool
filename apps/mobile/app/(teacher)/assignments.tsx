...
export default function TeacherAssignmentsScreen() {
	...
	// Queries
	const assignments = useQuery(api.assignments.listBySection, sectionId ? { sectionId } : 'skip')
	...
	const handleGrade = useCallback(
		async (submissionId: string, grade: string) => {
			try {
								...
								>
									...
									{sub.grade ? (
										...
										<Button
											...
											variant="outline"
											onPress={() =>
												openGradeDialog(sub._id, sub.studentName ?? sub.student ?? 'student')
											}
										/>
									)}
								</View>
	...
	return (
		<View style={{ flex: 1, backgroundColor: colors.bg }}>
			...
			>
				{showCreate && (
					<Card style={{ borderColor: '#7C3AED', borderWidth: 1.5 }}>
						...
						<View style={{ gap: 10 }}>
							...
							/>
							<Input placeholder="Due date" value={assignDueDate} onChangeText={setAssignDueDate} />
							<Button
				...
				{/* Loading state */}
				{!showCreate && assignments === undefined && sectionId && <SkeletonList count={3} />}
							...
							<View style={{ gap: 8 }}>
								...
								{total > 0 && (
									<View style={{ gap: 4 }}>
										...
										<View
											style={{ height: 6, backgroundColor: colors.surfaceAlt, borderRadius: 3 }}
										>
											<View
												style={{
													height: 6,
