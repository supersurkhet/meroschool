			...
			>
				{showUpload && (
					<Card style={{ borderColor: '#7C3AED', borderWidth: 1.5 }}>
								...
								return (
									...
									>
										<Ionicons
											name={cfg.icon}
											size={20}
											color={uploadType === type ? cfg.color : colors.textMuted}
										/>
										<Text
											style={{
												fontSize: 12,
						...
						<View style={{ gap: 10 }}>
							<Input placeholder="Title" value={uploadTitle} onChangeText={setUploadTitle} />
									...
									{(subjects as any[]).map((subj) => (
										<Pressable
											...
											style={{
												...
												borderRadius: 10,
												backgroundColor:
													selectedSubjectId === subj._id ? '#EDE9FE' : colors.surfaceAlt,
										...
										>
											<Text
												style={{
													fontSize: 13,
													fontWeight: '600',
													color: selectedSubjectId === subj._id ? '#7C3AED' : colors.textSecondary,
												}}
											>
							...
							<Button
								...
								loading={uploading}
								icon={
									!uploading ? <Ionicons name="cloud-upload" size={18} color="#FFF" /> : undefined
								}
							/>
						</View>
					</Card>
				...
				{/* Loading state */}
				{!showUpload && materials === undefined && selectedSubjectId && <SkeletonList count={3} />}
							...
							<View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
								...
								<View style={{ flex: 1 }}>
									...
									<Text style={{ fontSize: 12, color: colors.textSecondary, marginTop: 2 }}>
										{mat.subjectName ?? mat.subject ?? '—'} · {mat.sectionName ?? mat.class ?? '—'}{' '}
										· {mat.uploadedAt ?? mat.date ?? ''}
									</Text>
								</View>
								<View style={{ alignItems: 'flex-end' }}>
									<Badge
										...
										variant={
											matType === 'video' ? 'danger' : matType === 'pdf' ? 'primary' : 'success'
										}
									/>
									{mat.views != null && (
