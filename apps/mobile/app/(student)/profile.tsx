...
import { roleColors, useTheme } from '@/lib/theme'
					...
					<Card>
						<View style={{ gap: 8 }}>
							...
							<View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
								...
								<Text style={{ fontSize: 14, fontWeight: '600', color: colors.text }}>
									{(progress as any).assignments?.submitted ?? 0}/
									{(progress as any).assignments?.total ?? 0}
								</Text>
							</View>
							<View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
								<Text style={{ fontSize: 14, color: colors.textSecondary }}>Attendance Days</Text>
								<Text style={{ fontSize: 14, fontWeight: '600', color: colors.text }}>
									{(progress as any).attendance?.presentDays ?? 0}/
									{(progress as any).attendance?.totalDays ?? 0}
								</Text>
							</View>
						</View>
					</Card>
