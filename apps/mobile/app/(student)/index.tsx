...
function CircularProgress({ percentage, size = 80 }: { percentage: number; size?: number }) {
	...
	return (
		...
		>
			...
			/>
			<Text style={{ fontSize: 18, fontWeight: '800', color: colors.success }}>{percentage}%</Text>
		</View>
	)
}
...
export default function StudentDashboard() {
	...
	const isLoading =
		progress === undefined || testHistory === undefined || assignmentsData === undefined
							...
							<Card key={assignment._id} style={{ marginBottom: 10 }}>
								...
								>
									...
									<View style={{ alignItems: 'flex-end', gap: 4 }}>
										<Badge
											text={assignment.dueDate ?? assignment.dueDateLabel ?? '—'}
											variant="warning"
										/>
										<View
											style={{
						...
						<Card key={result._id ?? result.id} style={{ marginBottom: 10 }}>
							...
							>
								<View>
									...
									<Text style={{ fontSize: 13, color: colors.textSecondary, marginTop: 2 }}>
										{result.score}/{result.totalMarks ?? result.total} —{' '}
										{result.submittedAt ?? result.date}
									</Text>
								</View>
								<View
									style={{
								...
								>
									<Text style={{ fontSize: 16, fontWeight: '700', color: colors.success }}>
										{result.grade ??
											`${Math.round(((result.score ?? 0) / (result.totalMarks ?? result.total ?? 100)) * 100)}%`}
									</Text>
								</View>
							</View>
						</Card>
