...
export default function AssignmentsScreen() {
	...
	const tabs: { key: Tab; label: string; count: number }[] = [
		{
			key: 'pending',
			label: t('student.pending'),
			count: isLoading ? 0 : pendingAssignments.length,
		},
		{
			key: 'completed',
			label: t('student.graded'),
			count: isLoading ? 0 : completedAssignments.length,
		},
	]
			...
			<View style={{ flex: 1, backgroundColor: colors.bg }}>
				...
				<ScrollView contentContainerStyle={{ padding: 20, gap: 16, paddingBottom: 40 }}>
					...
					<Card>
						<View style={{ gap: 10 }}>
							<View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
								<Badge
									text={selectedAssignment.subjectName ?? selectedAssignment.subject ?? '—'}
									variant="primary"
								/>
								<Badge text={status.label} variant={status.variant} />
							</View>
							<Text style={{ fontSize: 13, color: colors.textSecondary }}>
								By {selectedAssignment.teacherName ?? selectedAssignment.teacher ?? '—'} · Due:{' '}
					...
					<View style={{ gap: 10 }}>
						...
						<Pressable
							onPress={() =>
								Alert.alert('Attach File', 'Select a photo, PDF, or document to attach.')
							}
							style={{
								flexDirection: 'row',
								alignItems: 'center',
								...
								>
									<View style={{ flex: 1 }}>
										...
										<Text style={{ fontSize: 13, color: colors.textSecondary, marginTop: 2 }}>
											...
											{a.score !== undefined ? `${a.score}/${a.totalMarks ?? a.total}` : ''}
											{(a.submittedAt ?? a.submittedDate)
												? ` · ${a.submittedAt ?? a.submittedDate}`
												: ''}
										</Text>
									</View>
