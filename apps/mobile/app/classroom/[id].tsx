...
export default function ClassroomScreen() {
	...
	const typeIcons: Record<string, { icon: keyof typeof Ionicons.glyphMap; color: string; bg: string }> = {
		...
		<View style={{ flex: 1, backgroundColor: colors.bg }}>
			...
			<ScrollView contentContainerStyle={{ padding: 20, gap: 10, paddingBottom: 40 }}>
				...
				{tab === "materials" && !isLoading && allMaterials.length === 0 && (
					<EmptyState icon="book-outline" title="No Materials" subtitle="No materials uploaded for this class yet" />
				)}

				{tab === "materials" && allMaterials.map((mat: any) => {
					...
					return (
						<Card key={mat._id}>
							<View style={{ flexDirection: "row", alignItems: "center", gap: 14 }}>
								...
								<Badge text={(mat.type ?? "pdf").toUpperCase()} variant={mat.type === "video" ? "danger" : mat.type === "pdf" ? "primary" : "success"} />
							</View>
						</Card>
					)
				})}
				...
				{tab === "tests" && !isLoading && publishedTests.length === 0 && (
					<EmptyState icon="document-text-outline" title="No Tests" subtitle="No tests available for this class" />
				)}

				{tab === "tests" && publishedTests.map((test: any) => (
					<Card key={test._id}>
						<View style={{ gap: 8 }}>
							<View style={{ flexDirection: "row", justifyContent: "space-between" }}>
								<Text style={{ fontSize: 15, fontWeight: "600", color: colors.text }}>{test.title}</Text>
								<Badge text={test.isPublished ? "Available" : "Draft"} variant={test.isPublished ? "success" : "default"} />
							</View>
							<Text style={{ fontSize: 13, color: colors.textSecondary }}>
